...
/**
 * CONTROLADOR ADMIN_FORMAS_REGIMENFISCAL
 *
 * @package  app
 * @extends  Controller_Admin
 */
class Controller_Admin_Formas_Regimenfiscal extends Controller_Admin
{
	/**
	 * BEFORE
	 *
	...
	public function action_index($search = '')
	{
		...
		$fiscales = Model_Sat_Tax_Regime::query()
		->where('deleted', 0);
		...
		if($search != '')
		{
			...
			$fiscales = $fiscales->where(DB::expr("CONCAT(`t0`.`name`)"), 'like', '%'.$search.'%');
		}
		...
		$config = array(
			...
			'total_items'    => $fiscales->count(),
			'per_page'       => $per_page,
			'uri_segment'    => 'pagina',
		);

		# SE CREA LA INSTANCIA DE LA PAGINACION
		$pagination = Pagination::forge('fiscales', $config);

		# SE EJECUTA EL QUERY
		$fiscales = $fiscales->order_by('id', 'desc')
		...
		$data['fiscales'] = $fiscales_info;
		$data['search']          = str_replace('%', ' ', $search);
		$data['pagination']      = $pagination->render();

		# SE CARGA LA VISTA
		$this->template->title   = 'Formas de pago';
		$this->template->content = View::forge('admin/regimenfiscal/index', $data, false);
	}
	...
	public function action_buscar()
	{
		...
		if(Input::method() == 'POST')
		{
			...
			if($val->run($data))
			{
				...
				Response::redirect('admin/formas_regimenfiscal/index/'.$search);
			}
			else
			{
				# SE REDIRECCIONA AL USUARIO
				Response::redirect('admin/formas_regimenfiscal');
			}
		}
		else
		{
			# SE REDIRECCIONA AL USUARIO
			Response::redirect('admin/regimenfiscal');
		}
	}
